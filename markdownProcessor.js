...
export function renderMarkdown(markdownContent) {
    ...
    for (const [placeholder, { language, code }] of codeBlocks) {
        ...
        const codeHtml = `<div class="code-block-wrapper">${copyButton}<pre><code class="language-${language}">${escapedCode}</code></pre></div>`;
    ...
    return [finalHtml.replace(/<p>\s*<\/p>/g, ""), sidebarContent];
}
