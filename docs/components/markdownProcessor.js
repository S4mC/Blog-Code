function obtainAttributes(e){const s=/(\w+)="([^"]+)"/g;let t,n="";for(;(t=s.exec(e))!==null;){const[,e,s]=t;e!=="src"&&(n+=` ${e}="${s}"`)}return n}function escapeHtml(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function processBalancedDelimiters(e,t){let s=e;const{findPattern:i,openChar:a,closeChar:r,processMatch:c,shouldProcess:l=()=>!0}=t,o=[];let n;for(;(n=i.exec(e))!==null;){const t={fullMatch:n,start:n.index,...n.groups||{}};let s=1,i=n.index+n[0].length,c=-1;for(;i<e.length&&s>0;){const t=e[i];if(t===a)s++;else if(t===r&&(s--,s===0)){c=i;break}i++}c!==-1&&(t.end=c+1,t.content=e.substring(n.index+n[0].length,c),l(t)&&o.push(t))}return o.reverse().forEach(t=>{const n=c(t,e);n!==null&&(s=s.substring(0,t.start)+n+s.substring(t.end))}),s}function processInlineCodeBlocks(e,t,n=!1){let c=0,s=e;const i=[];let a;const l=/`/g;for(;(a=l.exec(e))!==null;)i.push(a.index);const o=i.filter(t=>{const n=t>0&&e[t-1]==="`",s=t<e.length-1&&e[t+1]==="`";return!n&&!s}),r=[];for(let t=0;t<o.length-1;t+=2){const n=o[t],s=o[t+1];if(s!==0[0]){const o=e.substring(0,n),t=o.match(/ ([a-zA-Z0-9]+)$/);if(t){const o=t[1],i=e.substring(n+1,s);r.push({start:n-t[0].length+1,end:s+1,lang:o,code:i})}}}return r.reverse().forEach(e=>{let o=`INLINE_CODE_${c++}_CODE_INLINE`;n?o=`<code class="language-${e.lang}">${e.code}</code>`:t.set(o,{language:e.lang,code:e.code}),s=s.substring(0,e.start)+` ${o}`+s.substring(e.end)}),[s,t]}function processCodeBlocksAndTitles(e){let r=0,s=!1,i="",t=[],a=[],n=[],o=new Map;const c=e.split(`
`);for(let e of c){const l=e.trim();if(l.startsWith("```")){if(s){s=!1;let a=e.replace(l,""),c=t;if(t.length>0){const e=t.filter(e=>e.trim()!=="");if(e.length>0){let n=4+a.length;for(const t of e){let s=0;for(let e=0;e<t.length;e++)if(t[e]===" "||t[e]==="	")s++;else break;n=Math.min(n,s)}n>0&&n!==1/0&&(c=t.map(e=>e.trim()===""?e:e.substring(n)))}}const d=`CODE_BLOCK_${r++}_BLOCK_CODE`;o.set(d,{language:i,code:c.join(`
`)}),n.push(a+d),console.log("Processed code block:",o)}else s=!0,i=l.slice(3).trim(),t=[];continue}if(s)t.push(e);else{let t=e.replace(/</g,"").replace(/>/g,"").trim();if(t.startsWith("## ")||t.startsWith("### ")||t.startsWith("#### ")||t.startsWith("##### ")||t.startsWith("###### ")){let s=processInlineCodeBlocks(t.replace(/</g,"&lt;").replace(/>/g,"&gt;"),[],!0)[0];a.push(s),n.push(e)}else e.startsWith("#t ")?n.push(`<plain>${e.substring(3)}</plain>`):e.startsWith("<")?n.push(`${e}`):n.push(e)}}return[n.join(`
`),o,a]}function processMarkdownBlocks(e){function o(e){if(e.length===0)return e;const n=e.filter(e=>e.trim()!=="");if(n.length===0)return e;let t=4;for(const e of n){let s=0;for(let t=0;t<e.length;t++)if(e[t]===" "||e[t]==="	")s++;else break;t=Math.min(t,s)}return t>0&&t!==1/0?e.map(e=>e.trim()===""?"    ":e.substring(t)):e}const s=e.split(`
`);let t=[];function n(e){let n=e,t=[],i=0;for(;n<s.length;){const e=s[n],o=e.trim();if(o===":::"){if(i===0)break;i--,t.push(e)}else o.startsWith(":::")?(i++,t.push(e)):t.push(e);n++}return console.log("Block content before removing indentation:",t),t=o(t),console.log("Block content after removing indentation:",t),[t,n]}for(let e=0;e<s.length;e++){const i=s[e],o=i.trim();if(o.startsWith(":::conector")){t.push(""),t.push(`<div style="margin-left: 10px; padding-left: 20px; border-left: 3px solid #ffffff52; position: relative;" class="content-conector">`);const[s,o]=n(e+1),i=processMarkdownBlocks(s.join(`
`));t.push("");for(const e of i.split(`
`))t.push(e);t.push(""),e=o,t.push("</div>"),t.push("")}else if(o.startsWith(":::float-")){const s=o.substring(":::float-".length);t.push(""),t.push(`<div class="float-container" id="float-${s}"><button class="float-close">×</button>`);const[i,a]=n(e+1),r=processMarkdownBlocks(i.join(`
