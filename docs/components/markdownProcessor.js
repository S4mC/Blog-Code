...
`);for(let e of c){const l=e.trim();if(l.startsWith("```")){if(s){s=!1;let i=e.replace(l,""),c=t;if(t.length>0){const e=t.filter(e=>e.trim()!=="");if(e.length>0){let n=4+i.length;for(const t of e){let s=0;for(let e=0;e<t.length;e++)if(t[e]===" "||t[e]==="	")s++;else break;n=Math.min(n,s)}n>0&&n!==1/0&&(c=t.map(e=>e.trim()===""?e:e.substring(n)))}}const d=`CODE_BLOCK_${r++}_BLOCK_CODE`;a.set(d,{language:o,code:c.join(`
`)}),n.push(i+d)}else s=!0,o=l.slice(3).trim(),t=[];continue}if(s)t.push(e);else{let t=e.replace(/</g,"").replace(/>/g,"").trim();if(t.startsWith("## ")||t.startsWith("### ")||t.startsWith("#### ")||t.startsWith("##### ")||t.startsWith("###### ")){let s=processInlineCodeBlocks(t.replace(/</g,"&lt;").replace(/>/g,"&gt;"),[],!0)[0];i.push(s),n.push(e)}else e.startsWith("#t ")?n.push(`<plain>${e.substring(3)}</plain>`):e.startsWith("<")?n.push(`${e}`):n.push(e)}}return[n.join(`
