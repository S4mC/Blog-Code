...
`);for(let e of c){const l=e.trim();if(l.startsWith("```")){if(s){s=!1;let i=e.match(/^\s*/)[0],c=t;if(t.length>0){const e=t.filter(e=>e.trim()!=="");if(e.length>0){let n=4+i.length;for(const t of e){let s=0;for(let e=0;e<t.length;e++)if(t[e]===" "||t[e]==="	")s++;else break;n=Math.min(n,s)}n>0&&n!==1/0&&(c=t.map(e=>e.trim()===""?e:e.substring(n)))}}const l=`CODE_BLOCK_${r++}_BLOCK_CODE`;a.set(l,{language:o,code:c.join(`
