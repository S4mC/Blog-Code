...
function ThemeToggle() {
    ...
    useEffect(() => {
        ...
        document.documentElement.setAttribute(
            "monaco-theme",
            getCurrentThemeData()?.monacoTheme || "vs-dark"
        );
...
function SearchResults({ results, selectedIndex, onSelect, query }) {
    const selectedRef = useRef();
    const containerRef = useRef();

    // Auto-scroll to selected item within container
    useEffect(() => {
        if (selectedRef.current && containerRef.current && selectedIndex >= 0) {
    ...
    return html`
        <div class="search-results" ref=${containerRef}>
            ${results.map(
                (result, index) => html`
                    <div
                        key=${result.path}
            ...
            html`
                <div class="search-tags">
                    ${uniqueTags.map(
                        (tag) => html`<a class="tag" href="./index.html?tags=${tag}">${tag}</a>`
                    )}
                </div>
            `}
        </div>
...
function SearchBar({ onNavigateToEntry, isExpanded, onToggleExpand }) {
    ...
    return html`
        <div class=${`search-container ${isExpanded ? "mobile-expanded" : ""}`}>
            ${!isExpanded &&
            html`
                <button class="search-button" onClick=${handleToggleExpand} aria-label="Open search">
            ...
            <div class=${`search-bar ${isExpanded ? "expanded" : ""}`}>
                ${isExpanded &&
                html`
                    <button class="close-search" onClick=${handleToggleExpand} aria-label="Close search">
                        <svg
